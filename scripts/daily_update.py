...
KIND = "entry"                          # log-specific noun ("tip" | "entry")
KIND_LABEL = "Daily entry"
LOG_FILE = "docs/daily-log.md"          # file entries are appended to
POOL_FILE = "scripts/tips_pool.json"    # content pool (title/body/command)
COMMIT_PREFIX = "docs: daily ledger entry"
LOG_HEADER = [
...
def existing_dates() -> set[dt.date]:
    ...
    return {dt.date.fromisoformat(m) for m in re.findall(r"^## (\d{4}-\d{2}-\d{2})", txt, re.M)}
